...
do 
    {
        ...
        #-----------------------------------------------------------------------
        # BAM File Processing and downsampling to 50
        #-----------------------------------------------------------------------
            ...
            # Get the region length from the BED file
            region_length=$(awk -v coord="${Coordinate}" 'BEGIN{split(coord,a,":"); split(a[2],b,"-"); print b[2]-b[1]}')

            # Create a temporary file for the filtered reads
            samtools view -H "${workdir}/${Barcode}/temp.bam" > "${workdir}/${Barcode}/filtered.sam"

            # Filter for fully spanning reads and downsample to 50 reads
            samtools view -@ 20 "${workdir}/${Barcode}/temp.bam" "${Coordinate}" | \
                awk -v len="${region_length}" '
                ...
                head -n "${downsample_to}" >> "${workdir}/${Barcode}/filtered.sam"

            # Convert back to BAM format
            samtools view -@ 20 -b "${workdir}/${Barcode}/filtered.sam" > "${workdir}/${Barcode}/${Episode}.bam"

            # Index the final BAM file and cleanup
            samtools index "${workdir}/${Barcode}/${Episode}.bam" && \
            rm "${workdir}/${Barcode}/temp"* "${workdir}/${Barcode}/filtered.sam"

            echo "$(date '+%Y-%m-%d %H:%M:%S') - Bam files merged and downsampled to ${downsample_to} reads!"
        #-----------------------------------------------------------------------
        ...
        if [[ ! "$Variant1" =~ chr ]] || [[ ! "$Variant2" =~ chr ]]; then
            ...
            python "${SCRIPT_PATH}/test_call.py" \
                "${workdir}/${Barcode}/${Episode}.bam" \

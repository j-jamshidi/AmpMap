...
merge_bam_files() {
    ...
    log "Merging BAM files for ${barcode}, sample ${episode} with delicate downsampling to ${DOWNSAMPLE_TO} reads..."
    ...
    # Get the region length from the coordinate
    local region_length
    region_length=$(echo "${coordinate}" | tr -d ' ' | tr ':' '\t' | sed 's/-/\t/g' | awk '{print $3-$2}')
    
    log "Region length: ${region_length} bp, filtering for fully spanning reads..."
    
    # Create a temporary file for the filtered reads and ensure directory exists
    docker run --rm -v "${WORKDIR}:/output" --entrypoint sh javadj/ontampip:latest \
        -c "mkdir -p /output/${barcode} && samtools view -H /output/${barcode}/temp.bam > /output/${barcode}/filtered.sam"
    
    # Filter for fully spanning reads and downsample to specified number
    docker run --rm -v "${WORKDIR}:/output" --entrypoint sh javadj/ontampip:latest \
        -c "samtools view -@ 6 /output/${barcode}/temp.bam '${coordinate}' | awk -v len='${region_length}' -v downsample='${DOWNSAMPLE_TO}' '
        function abs(x) {return x < 0 ? -x : x}
        {
            # Check if read spans the entire region (using CIGAR string)
            cigar=\$6
            readlen=0
            while(match(cigar, /[0-9]+[MIDNP=X]/)) {
                op=substr(cigar,RSTART+RLENGTH-1,1)
                num=substr(cigar,RSTART,RLENGTH-1)
                if(op ~ /[M=X]/) readlen+=num
                cigar=substr(cigar,RSTART+RLENGTH)
            }
            if(abs(readlen-len) <= 100) print
        }' | head -n '${DOWNSAMPLE_TO}' >> /output/${barcode}/filtered.sam"
    
    # Convert back to BAM format
    docker run --rm -v "${WORKDIR}:/output" --entrypoint samtools javadj/ontampip:latest \
        view -@ 6 -b /output/${barcode}/filtered.sam -o /output/${barcode}/${episode}.bam
    
    # Index the final BAM file and cleanup
    docker run --rm -v "${WORKDIR}:/output" --entrypoint samtools javadj/ontampip:latest \
        index /output/${barcode}/${episode}.bam || error_exit "Failed to index final BAM"
    
    rm "${WORKDIR}/${barcode}/temp"* "${WORKDIR}/${barcode}/filtered.sam" 2>/dev/null || true
    log "BAM files merged and delicately downsampled to ${DOWNSAMPLE_TO} spanning reads!"
}
